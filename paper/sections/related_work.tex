...
its instantiation against a \emph{sampling-based} detector: the manipulated quantity is an
entropy over meaning-clustered samples rather than a softmax or a learned detector's logit;
the victim is a hallucination detector rather than an attack detector; the perturbation is
a question rewrite rather than a pixel budget or a hidden-state intervention; and semantic
invariance therefore has to be \emph{certified} by an equivalence gate rather than granted
by an $\ell_p$ ball. Most relevant of all, CORVUS red-teams single-pass hallucination
...
paraphrases is an established primitive, and directional manipulation of a confidence signal
under prediction-preservation is an established family. To our knowledge the combination has
not been pointed at a \emph{sampling-based} hallucination detector --- one whose score is an
entropy over meaning-clustered samples rather than a classifier output --- from the input
side alone (we evaluate the false-alarm direction here and build the hide direction as its
mirror). The bidirectional-NLI equivalence gate is our design choice for
