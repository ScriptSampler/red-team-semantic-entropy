...
\emph{same} quantity as the fair pool's, only far less precisely. The attacked subset's
$0.184$ nats and AUROC $0.579$ carry a bootstrap interval of [$0.416$, $0.728$] that
comfortably contains the fair pool's $0.704$ [$0.653$, $0.753$]; the two are statistically
consistent, and the gap between them is sampling error over roughly a thirtieth as many
ranked pairs, not evidence that the detector behaves differently on attacked targets. We
therefore quote the fair pool's value when we characterise the detector, and we do not
present the attacked subset's as a rival measurement of it.
...
plug-in entropy is not just bounded by $\ln 10$; it is confined to the $39$ values obtainable
...
This is a property of the estimator at the deployed sample budget, not of our attack: it
applies identically to any method evaluated with a fixed-$\delta$ criterion on this score.
...
end-to-end and reports correctly, and nothing else. We deliberately quote no point estimate
from it, for two independent reasons. The net confidence interval includes zero at that
sample size, so the effect is not claimed. And the per-target statistics that pass would
otherwise invite an arm-to-arm reading from are quantised far too coarsely to carry one: with
$K{=}5$ benign draws per target, a target's percentile within its own benign-move
distribution can take only six values, so a difference of a few percentile points between
clusterer arms is smaller than a single benign draw, and the arms cannot be ordered at all at
that resolution. Which arm the judge's verdict resembles is therefore still open, and the
