...
$80$ false-alarm targets, $39$ finish \emph{exactly} at the ceiling and $8$ began there (so
no upward move was ever possible), whereas none of the $17$ hide targets scored so far
reaches the floor---though that last comparison is on a hide cell that was truncated
mid-campaign at $17$ of $80$, and we restate it with its final $n$ when that cell
completes.

We label the population of every quantity we report, because the two differ materially and
conflating them is easy: the fair pool gives a clean correct-versus-wrong separation of
$0.463$ nats and AUROC $0.704$, whereas the \emph{attacked subset} gives $0.184$ nats and
AUROC $0.579$. The attacked subset is a selected, partly truncated sample and is not a
description of the detector; wherever we characterise the detector we use the fair pool, and
wherever we describe what the attack did we say so explicitly.
