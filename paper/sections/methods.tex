...
gate hop is therefore not assumed away but measured directly, in the answer-flip
subcategory of the evaluation below. The search terminates on a score threshold or an
iteration budget of $20$ rounds, which is $1 + 20 \times P \times B \approx 181$ objective
evaluations per target. That count is the attacker's search budget, and the benign noise
floor of $m{=}50$ random NLI-passing paraphrases is matched against it analytically, by the
exceedance test of Section~\ref{sec:experiments}, rather than by setting $m$ equal to it. We
...
deliberately attach no count to that last statement here: the hide cell is still filling
...
from unflagged to flagged. Those are the operationally meaningful events. Fixing that
...
attack's net effect under each as a bracket, with the shared-NLI arm as the confounded upper
bound and exact-match as a strict lower bound. Alongside it we report the survival ratio
between them, shown as a diagnostic only and not as a claim statistic. We find that no cheap
