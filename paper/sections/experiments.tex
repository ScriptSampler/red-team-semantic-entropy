...
manufactures by construction---an interval that contains the operative replication figure
...
that budget---under each clustering relation the campaign instantiates. ($m$ is
...
one. The LLM-judge (Qwen2.5-7B-Instruct), victim- and NLI-independent and
...
per target---the original question and the $\sim$180 candidate paraphrases it proposes
(Section~\ref{sec:methods})---so any comparison against individual benign draws is
upward-biased by construction: under the null a max-of-181 sits at $\sim$$99\%$ of an
individual-draw distribution, not $50\%$. We
...
$\mathbb{E}[K] = m/(A{+}1)$, where $A$ is the attacker's search budget---the number of
scored evaluations the maximum ranges over, $A \approx 181$, one more than the
$\sim$180 candidate paraphrases because the original's own score is among them---and is
distinct from the per-question sample budget $N$, which is $10$ throughout this section.
It is not $10$ throughout the paper: Sections~\ref{sec:discussion}
and~\ref{sec:limitations} characterise the clean detector on the fair pool at $N{=}20$ and
$N{=}40$ as well. Fewer exceedances than that is evidence for the attack; the
...
against the true simulated null---an oracle the deployed test does not have---reaches $0.66$
at level $0.056$ and $0.84$ at $0.064$ for those two cells. The difference between the two
...
conservatism---and, unlike the oracle, it delivers a level the pipeline can state without
...
$80$ score-independently selected correct-answer targets the optimiser was run on---and not
on the fair pool used to characterise the clean detector (Section~\ref{sec:methods}). The two
are nested: these targets are a prefix of the fair pool's $200$ correct answers, so the
distinction is not between disjoint question sets but one of scope---a single correctness
stratum, under the optimiser's own selection---and no number is carried between them.
...
(\texttt{qb\_565}, \texttt{qz\_3393}, \texttt{qb\_2689}) have \emph{no} benign arm---the
feasibility gate rejected all $250$ attempts---and are excluded rather than absorbed. They are
...
measured $+0.006$ nats as-is and $+0.001$ under~(d)---disclosed, not corrected. The
...
$11$ no-op targets are the three with an empty benign arm, which are already outside the $77$,
so only the remaining eight are excluded here. The paired net therefore credits the attack with
...
the attack's judge score is unselected, and that asymmetry---not the convention---is what puts
the judge column below zero.}
...
than a weaker statistic---re-cut to the oracle's level the two agree to three decimals, as
above---but it is the operating point we actually ship. The ordering that drove the choice
...
design is about $99$ GPU-hours. The deviation still holds---an analytic null that costs nothing
is preferable to a four-day matched arm---but the trigger we recorded was $2.3\times$ too
large, and a reader checking whether the rule was tuned is entitled to the corrected number.
And the tie rule moved from
...
attacked pool}---all $80$ score-independently selected correct-answer targets, the
pre-registered $n{\geq}80$ met by this population and by no other quantity in this section---and
scores each target's outcome under three clustering relations, against $m{=}50$ benign feasible
...
That total is itself a tie-break realisation and we quote it as one rather than beside a median
$p$ as though it were fixed: at the seed-$0$ draw it is $30$, $383$ and $636$ against an
expectation of $20.4$, and the medians over the same $101$ draws the $p$-values summarise are
$28$, $383$ and $652$. The shared-NLI total is the one that moves, over $[16,41]$ across those
draws, so the most attack-favourable realisations do put it just below expectation---and that
...
$n_{\mathrm{eff}}{\approx}122$ random feasible paraphrases---fewer than the budget it spent.
...
is $80$; every benign-referenced statistic---the exceedance test and the paired net alike---runs
on $77$, because three targets have no benign arm at all; convention~(c) runs on $69$; and the
full $m{=}50$ budget was achieved on $71$. The three empty arms
...
feasible paraphrase---joint evidence that no feasible paraphrase of those
...
They are excluded and stated, never absorbed into a denominator of $80$. Six further arms are short; the resulting inflation of the
...
are weaker evidence than the NLI one, and the NLI arm---confounded though it is as an effect
estimate---is the only place where this test is asking the question it was designed to ask.
...
against an expectation of $1.93$---and they do so in all $101$ tie-break realisations, not on
average---which pushes $S$ down and $p$ toward rejection \emph{in the attack's favour}. It is
...
test still does not reject at any realisation. The other two arms show the reverse---their
no-op targets donate far \emph{more} exceedances than expected---so nothing about this
violation is shared across the three.
...
quantity. The index-matched row of Table~\ref{tab:nullcontrol} sidesteps the need to, by
holding the selection rule fixed across both sides, and it covers zero.
...
stable to the convention---it is negative under (b), (b$'$), (c) and (d)---and to the bootstrap,
agreeing with a $t$-test ($p{=}0.013$) and a signed-rank test ($p{=}0.007$).
...
a query chosen on NLI. The benign arm is handed a search the attack never ran---worth
$+0.49$ nats of selection premium in that arm, and the NLI-selected benign candidate attains the
...
large movers. This is the same defect the null control exists to
correct---a maximum compared against a differently-budgeted maximum---reappearing one level
down, per arm rather than per design, and running \emph{against} the attack rather than for it,
which is very likely why review aimed at pro-attack bias did not surface it. \textbf{The
defensible reading of the adjudicator arm is that its index-matched interval covers zero}---and
by the rule stated above we report it as covering zero rather than as null, which an interval
spanning $[-0.112, +0.150]$---about a quarter of a nat---does not license: at this $n$, on this
population,
