...
that discrete semantic entropy fails as a detector. It is a claim about one region of its range, and
the counts that establish it are within-stratum frequencies measured on a sub-sample of that
same pool---the attack campaign's $80$ correct-answer targets, scored clean, before anything
is done to them. There, a tenth of the correct
answers ($8/80$) already sit at the estimator's maximum, a quarter ($21/80$) sit within the
top tenth of its range---a region containing just two of the estimator's $39$ attainable
values, so those $21$ targets occupy one of two points---and it is exactly into that compressed
region that a false alarm must be pushed. Under our own success criterion they cannot register
a success at all, whatever the paraphrase: those two points lie $0.139$ nats apart, so the
criterion's $\delta{=}0.25$ is out of reach from the lower one and there is nowhere above the
upper one to go (Methods).

The sharpest way to put the finding is therefore not ``we constructed an attack that defeats
semantic entropy'' but ``discrete semantic entropy has so little usable range at the top of
its scale that the false-alarm direction is largely unmeasurable in the detector's own
units.'' The qualifier is load-bearing and we do not drop it: the ceiling behind this reading
is shared with the likelihood-weighted estimator, but the lattice that makes the range so
coarse is not (Section~\ref{sec:methods}). The
...
of correct ones---but we are careful not to over-read that. AUROC is a rank statistic and
