...
def write_report(log, args, recs, top, cache, SRC, NAMES, packs, point, boot, mech,
    ...
    log(f"   The AUROC gain shrinks from +{F['gain_direct']:.3f} to +{F['gain_ll']:.3f} "
        f"[{F['gain_ll_lo']:+.3f}, {F['gain_ll_hi']:+.3f}], which covers zero;")
    log(f"   the low-FPR pAUC does not fall with budget, it rises by {F['pauc10_ll']:+.3f}, so the")
    log("   sentence \"partial AUROC below a 10% false-alarm rate is 0.145 at N=10 and 0.126")
    log(f"   at N={top}\" is a measurement of the provenance step and not of the budget.")
    log("")
    log("---")
    log("")
    ...
    log("")
    log("**Sign and magnitude.** The replay reads HIGHER than the direct cache on the floor")
    log(f"({F['step_floor'] * 100:+.1f} points, {F['step_floor_rel']:+.0%} relative) and on AUROC "
        f"({F['step_auroc']:+.3f}), and LOWER on")
    ...
    log("one of those intervals covers zero. None of them is a small effect on the scale of")
    ...
    log(f"generations fixed and varying only the subset draw, the {top}-sample budget ranks")
    log(f"better in {R - int(F['n_ties_40'])} of {R} draws, so the direction is consistent. What the")
    ...
    log("- and it buys almost nothing: TPR at a matched 5% is")
    log(f"  {point['replay10']['m05']:.1%} at N=10 against {point[meas]['m05']:.1%} at N={top}.")
    log("")
    ...
    log("-- and (b) a small real generation-run difference between June and August. Both")
    log("point the same way: **the replay is the sounder of the two measurements, and the")
    log("direct N=10 cache is the row whose provenance deserves the scrutiny.** It is a")
    log("single realisation, generated on different hardware-days, and it is the only row")
    log("in the budget table that cannot be reproduced from the checkpoint.")
    log("")
    ...
    log("")
    log("**Wrong 1: that fall is the provenance step, not the budget.** Like-for-like the")
    log("low-FPR pAUC does not fall at all -- it rises:")
    log("")
    ...
    log(f"Direct -> N={top} on the pAUC below 10% is {F['pauc10_direct_step']:+.4f} -- the claim's "
        "quoted -0.019. Replay")
    log(f"N=10 -> N={top} is {F['pauc10_ll']:+.4f}: opposite sign, and {F['pauc10_ratio']:.1f}x smaller. "
        "The quoted fall measures which")
    log("cache the N=10 row came from, not what the budget did.")
    log("")
    ...
    log("DOES NOT SURVIVE AS WRITTEN.** The fall it cites is the provenance step with the")
    log(f"sign reversed: like-for-like the low-FPR pAUC rises by {F['pauc10_ll']:+.3f}. What is left is a")
    log("decomposition of an undetectable gain, which is not a finding. If the paper wants a")
    ...
    log("provenance and does not need an AUROC at all: the floor falls monotonically with")
    log(f"budget ({fl['replay10']:.1%} -> {fl['replay20']:.1%} -> {fl[meas]:.1%}), a 5% false-alarm budget")
    log(f"goes from unbuyable to buyable, and the deterministic rule it unlocks catches")
    log(f"{point[meas]['det05']:.1%} of hallucinations -- against the {point['replay10']['m05']:.1%} a coin-flip "
        "between two N=10 thresholds")
    log("already delivers at the same false-alarm rate, at a tenth of the cost. The")
    log("rebuttal \"just raise N\" is answered by granularity, not by discrimination --")
    log("which is the paper's thesis anyway.")
    log("")
