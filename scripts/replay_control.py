...
_PROVENANCE_PARAS = (
    ...
    "**WITHDRAWN, 2026-08-19; reason corrected 2026-08-26.** The N=40 floor's interval --"
    " BOTH candidates. The reason is NON-IDENTIFICATION and it uses no population model:"
    " once the ceiling atom empties, whether the top score this pool reached is the top of"
    " the population's support cannot be decided at n=200, and the two readings differ by"
    " more than two orders of magnitude. The coverage figures are BRANCH-CONDITIONAL and"
    " must never be quoted without their population -- under the calibrated Ewens fit"
    " Wilson covers 53.67% and the question bootstrap 0.00% at nominal 95%; under the zero"
    " branch, 95.06% and 100%. This banner said \"because the estimand stops existing when"
    " the ceiling atom empties\" until 2026-08-26; section 13 of the ruling retracts that,"
    " because it holds in one branch only. Quote the point, 2.0%, with no interval, and"
    " quote the at-cap mass 0/200 = 0.0% [0.0%, 1.9%] beside it -- that one is valid in"
    " both branches. Section 2c, and `results/n40_floor_estimator_ruling.md` sections 8.4"
    " and 13. The `achieved FPR, B% budget` brackets are"
...
def write_report(log, args, recs, top, cache, SRC, NAMES, packs, point, boot, mech,
    ...
    log("")
    log("WHAT THE PRECISION DOES NOT TOUCH IS THE RULING -- but the REASON is not the one")
    log("this section used to give. It said \"both candidates fail and the estimand")
    log("dissolves when the ceiling atom empties\", and called that model-free. It is")
    log("neither. Section 13 of `results/n40_floor_estimator_ruling.md` retracts it: both")
    log("halves hold only in the branch where the population CAN produce 39 mutually")
    log("inequivalent answers out of 40. In the other branch the pool max is the")
    log("population's top rung, 2.0% is an ordinary binomial proportion, Wilson covers at")
    log("95.06% and the bootstrap at 100%. The data does not exclude that branch: 0/200 is")
    log("a p=0.1175 outcome under the fitted model, and the model-free bound [0%, 1.88%]")
    log("contains both zero and the model's 1.065%.")
    log("")
    log("THE REPLACEMENT REASON IS MODEL-FREE AND STRONGER: tau_top IS NOT IDENTIFIED. It")
    log("equals 2.0% if P(K >= 39) is exactly zero and can be arbitrarily smaller if it is")
    log("not, and n=200 cannot distinguish those cases. An estimand whose value moves by")
    log("more than two orders of magnitude across a hypothesis the sample cannot test does")
    log("not have a confidence interval -- and that argument needs neither the Ewens fit")
    log("nor any coverage simulation. A reader who rejects the model should reject the")
    log("53.7%, keep the withdrawal, and quote the point alone; the withdrawal does not")
    log("rest on the model, but the phrase \"both estimators fail\" does.")
    log("")
    log("COVERAGE IS BRANCH-CONDITIONAL, so never quote a figure from the table above")
    log("without naming the population it was measured under. At nominal 95%, from")
    log("section 8.4 of the ruling:")
    log("")
    log("| population | P(pool max is the top rung) | Wilson | question bootstrap |")
    log("| --- | --- | --- | --- |")
    log("| calibrated Ewens (tau_top = 0.2726%) | 42.1% | 53.67% | 0.00% |")
    log("| zero branch (tau* = 2.0%) | 98.24% | 95.06% | 100% |")
    log("")
    log("A \"42% of the time\" figure sat in this section unqualified until 2026-08-26; it")
    log("is the FIRST row only, and under the second it is 98%. The same conditionality")
    log("applies to the m-out-of-n repair argument that stood beside it: resizing the")
    log("bootstrap cannot fix the interval because the statistic falls like c/m as the")
    log("pool grows -- but that decay is itself a property of the first branch, so the")
    log("argument is a reason not to trust the bootstrap THERE, not a second independent")
    log("ground for the withdrawal. The ground for the withdrawal is non-identification.")
    log("")
